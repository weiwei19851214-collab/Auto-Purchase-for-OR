...
- 本地配置保存一次后应可长期复用。只有配置确实缺失或当前浏览器存储被清空
  ...
  就把配置标记为未保存。
- `OPOM_BASE_URL` 默认值为 `http://20.2.209.2`。测试本地 OPOM 时允许
...
- 进入充值页后不要预先关闭 Auto Top-Up 规则。这个逻辑当前收益低、误判率高，
  已从主流程移除。
- Auto Top-Up 配置必须走弹窗内的短路径，不要围绕外层卡片堆复杂兜底：
  点击 `Manage`/`Enable` 后，如果弹窗只显示 `Enable auto top up` 开关且开关
  关闭，应立即点击弹窗里的开关；可用 DOM 识别 `role="switch"` /
  `aria-checked` / `data-state`，也可按开关矩形位置点击，但不要长时间等待完整
  表单先出现。
- Auto Top-Up 开关打开后，不管是本次刚打开还是上次已经打开，都应马上识别
  弹窗内 `When credits are below` 和 `Purchase this amount` 两个输入框，并快速
  写入请求值，例如“低于 `100` 充值 `150`”。只要弹窗字段可读写，就优先以弹窗
  字段为准，不要等待外层 overview 文案刷新。
- 外层 Credits 页面右侧 Auto Top-Up 卡片如果显示已启用但数字不符合请求
  （例如页面显示低于 `2` 充值 `25`，而任务要求 `100` / `150`），不要直接判定
  失败；这类情况优先进入 `Manage` 弹窗修正字段并保存。只有弹窗修正后仍无法
  回读目标值，才作为配置失败处理。
- Auto Top-Up 相关改动优先对齐 `main` 分支已有短流程：短等表单、读开关、
  必要时点击开关、填两个输入框、保存、再回读。不要新增大段弹窗壳等待、
  过度 scoring 或多层兜底；如果逻辑看不清，先比较 `main` 后再做最小修复。
